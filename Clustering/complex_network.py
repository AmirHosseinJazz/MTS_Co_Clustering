...
def main(
    ...
    try:
        ...
        return
    try:
        assert distances.shape[0] == real_data.shape[0]
    except AssertionError:
        print("Distances shape is not equal to real data shape")
        return
    ...
    G.add_nodes_from(range(len(real_data)))
    ...
    save_dir = f"./experiment_raw_data/experiment_raw_data_{exp_id}"
    ...
    if clustering_algorithm == "louvain":
        ...
        info_pd.loc[6] = ["Modularity", evaluate_partition(G, partition)]
        info_pd.loc[7] = [
            "Silhouette Score",
        ...
        # Evaluate the partition
        info_pd.loc[8] = ["Modularity", modularity]
        info_pd.loc[9] = ["Num Communities", len(communities)]
        info_pd.loc[10] = [
            "Silhouette Score",
            evaluate_partition_sil(real_data, partition),
        ]
...
if __name__ == "__main__":
    ...
    if args.data_source == "29var":
        real_data, feature_names = load_data(
            ...
            break_to_smaller=args.break_data,

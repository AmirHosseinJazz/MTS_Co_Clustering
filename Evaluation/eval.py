...
def marginal_distribution_comparison(
    real_data, generated_data, model_name, feature_names
):
    ...
    for feature_index in range(num_features):
        ...
        plt.legend()
        plt.savefig(
            f"./marginal_dist/{model_name}/feature_{feature_names[feature_index]}_distribution_comparison.png"
        )
        ...
        ks_stats.append(
            {
                "feature_index": feature_names[feature_index],
                "ks_stat": ks_stat,
                "p_value": p_value,
            }
        )
...
def multivariate_distribution_comparison(
    ...
    plt.bar(
        range(len(generated_cov.flatten())),
        generated_cov.flatten(),
        alpha=0.5,
        label="Generated",
    )
...
def temporal_dynamics_comparison(
    real_data,
    generated_data,
    model_name,
    feature_names,
    ...
    for feature_index in range(num_features):
        ...
        plot_acf(
            ...
            lags=max_lag,
            ax=plt.gca(),
            title=f"ACF - Real Data (Feature {feature_names[feature_index]})",
        )
        ...
        plot_acf(
            ...
            lags=max_lag,
            ax=plt.gca(),
            title=f"ACF - Generated Data (Feature {feature_names[feature_index]})",
        )
        ...
        plot_pacf(
            ...
            lags=max_lag,
            ax=plt.gca(),
            title=f"PACF - Real Data (Feature {feature_names[feature_index]})",
        )
        ...
        plot_pacf(
            ...
            lags=max_lag,
            ax=plt.gca(),
            title=f"PACF - Generated Data (Feature {feature_names[feature_index]})",
        )

        plt.tight_layout()
        plt.savefig(
            f"./temporal_dynamics/{model_name}/temporal_dynamics_feature_{feature_names[feature_index]}.png"
        )
...
def dwt(
    ...
    for i in range(num_cases):
        ...
        generated_sequence = generated_data[
            i % generated_data.shape[0]
    ...
    pd.DataFrame(dtw_distances).to_csv(
        f"./sequence_level/{model_name}/dtw_distances.csv", index=False
    )
    pd.DataFrame(
        [np.mean(dtw_distances), np.median(dtw_distances), np.std(dtw_distances)],
        index=["Mean", "Median", "Standard Deviation"],
        columns=["Value"],
    ).to_csv(f"./sequence_level/{model_name}/dtw_stats.csv")
...
def discrete_frechet_dist(P, Q):
    ...
    def c(i, j):
        ...
        if i == 0 and j == 0:
            ...
            ca[i, j] = max(
                min(c(i - 1, j), c(i - 1, j - 1), c(i, j - 1)), euclidean(P[i], Q[j])
            )
        else:
            ca[i, j] = float("inf")
        return ca[i, j]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name", type=str, default="TimeGAN_model4", help="Name of the model"
    )

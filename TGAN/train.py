...
def embedding_trainer(
    ...
    for epoch in range(params["embedder_epoch"]):
        ...
        with torch.no_grad():
            _, E_loss0_test, E_loss_T0_test = model(
                X=X_mb, T=T_mb, Z=None, obj="autoencoder"
            )
...
def joint_trainer(
    ...
    for epoch in range(params["supervisor_epochs"]):
        for X_mb, T_mb in dataloader:
            ...
            Z_mb = torch.rand(
                (params["batch_size"], params["max_seq_len"], params["Z_dim"])
            ).to(device)
            model.zero_grad()
        ...
        wandb.log({"E_loss": E_loss.item()}, step=epoch)
        wandb.log({"G_loss": G_loss.item()}, step=epoch)
        wandb.log({"D_loss": D_loss.item()}, step=epoch)

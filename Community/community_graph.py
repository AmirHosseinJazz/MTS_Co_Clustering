...
def evaluate_partition_sil(data, partition):
    ...
    if len(set(labels)) > 1 and len(set(labels)) < len(data):
        ...
        print("Not enough clusters for Silhouette Score. There must be at least 2 clusters.")
...
def main(distance_metric, connectivity, k_neighbors, epsilon,normalize):
    ...
    if connectivity == 'epsilon':
        ...
        knn_graph = kneighbors_graph(data, n_neighbors=k_neighbors, metric=distance_metric, mode='distance', include_self=False)
    ...
    exp_id=str(datetime.now().strftime('%Y-%m-%d_%H-%M-%S')).replace(' ','').replace(':','')
    ...
    np.savetxt('./experiment_{}/partition.csv'.format(exp_id), np.array(list(partition.values())), delimiter=',')
    ## save graph as gexf
    nx.write_gexf(G, './experiment_{}/graph.gexf'.format(exp_id))

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run community detection on generated data with various configurations.")
    parser.add_argument('--distance', type=str, default='euclidean', choices=['euclidean', 'manhattan', 'cosine'],
                        help='The distance metric to use (euclidean, manhattan, cosine). Default is euclidean.')
    parser.add_argument('--connectivity', type=str, default='epsilon', choices=['epsilon', 'knn'],
    ...
    parser.add_argument('--model_name', type=str, default='TimeGAN_model1',help='Name of the model used to generate the data. Default is TimeGAN_model1.')
    args = parser.parse_args()
    main(args.distance, args.connectivity, args.k_neighbors, args.epsilon, args.normalize)